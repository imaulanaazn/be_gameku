...
const path = "/v1/sync-desc-lapakgaming";
...
const main: RequestHandler = async (req, res) => {
    ...
    for (const game of games) {
        let desc = `<p>Cara Top Up ${game.name} di Gasskeun Topup :</p>\n<p><br /></p>`;
        ...
        desc += `<p>${game.type === "topup" ? "2" : "1"}. Pilih Nominal Denom yang kamu inginkan</p>`;
        desc += `<p>${game.type === "topup" ? "3" : "2"}. Selesaikan pembayaran</p>`;

        if (game.type === "topup") {
            desc += `<p>4. Denom akan ditambahkan ke akun ${game.name} kamu</p>`;
        } else {
            desc += `<p>3. Voucher akan dikirim</p>`;
        }
