...
export default class ExcelService<T> {
  async createExcel({
    ...
    const columns: Partial<Column>[] = columnsConfig.map((config) => ({
      ...
      ...(config.centered
        ? { alignment: { horizontal: "center", vertikal: "center" } }
        : {}),
    }));
    ...
    const response = await fetch(
      logo.value ||
        "https://firebasestorage.googleapis.com/v0/b/gameku-topup.appspot.com/o/assets%2Fconfig%2Fba2c0309-5452-4e18-9e4b-dfe95d96c712.png?alt=media&token=1ff6550c-f0df-4672-99b3-0af079507de5"
    );
    ...
    const metadata = [
      ...
      {
        label: "Periode",
        value: `${dayjs(startAt).format("YYYY-MM-DD HH:mm:ss")} - ${dayjs(
          endAt
