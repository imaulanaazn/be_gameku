...
const main: RequestHandler = async (req, res) => {
  ...
  if (payment.cd === "GAMEKU_CASH") {
    ...
    if (!reqbalance.ok) {
      throw new BusinessError(
        "Metode pembayaran Gameku Cash hanya bisa digunakan ketika login",
        ErrorType.Authorization
      );
    }
  ...
  let voucher;
  if (body.promoCode) {
    voucher = await voucherService.findAvailablePromoBypromoCode(
      body.promoCode
    );
    if (!voucher) {
      throw new BusinessError(
    ...
    if (body.userId || body.serverId) {
      ...
      if (orderDetail.length > 0) {
        throw new BusinessError(
          "Kode promo pernah sudah digunakan",
    ...
    if (voucher.gameId && !product.gameId) {
      throw new BusinessError(
        "Kode Promo tidak valid untuk game ini",
        ErrorType.BadRequest
      );
    }
    ...
    if (voucher && product.price * body.quantity >= voucher.minPurchase) {
      if (voucher.discountType === DiscountType.PERCENTAGE) {
        const disc =
          (voucher.discountValue / 100) * (product.price * body.quantity);
  ...
  let amount = Math.ceil(product.price * body.quantity - discount + uniqueCode);
  ...
  if (payment.cd === "GAMEKU_CASH" && amount > balance.value) {
    throw new BusinessError(
      "Saldo kamu tidak mencukupi untuk melakukan transaksi",
      ErrorType.BadRequest
    );
  }
  ...
  //           tokopayInvoices.error_msg ||
  //             "Ada kesalahan ketika membuat pembayaran"
  //         ),
  //       },
  //     });
  ...
  //       const cashtag = body.cashtag.startsWith("$")
  //         ? body.cashtag
  //         : "$" + body.cashtag;
  ...
  //   });
  // } else
  if (payment.providerCd === "INTERNAL") {
    try {
      await fetch(
  ...
  } else if (payment.providerCd === "MANUAL") {
    const now = dayjs();
    const expiredAt = now.add(2, "hour");
