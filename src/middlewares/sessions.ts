...
export const authWebhookTokopay: RequestHandler = async (req, res, next) => {
  ...
  if (remoteAddr === config.tokopayIp) {
    ...
    const merchantId = configTokopay.find(
      (item) => item.cd === "tokopay_merchant_id"
    );
    const secretKey = configTokopay.find(
...
export const authDigiflazzOrder: RequestHandler = async (req, res, next) => {
  ...
  const username = sysConfig.find(
    (item) => item.cd === "username_seller_digiflazz"
  ).value;
  const apiKey = sysConfig.find(
    (item) => item.cd === "api_key_seller_digiflazz"
  ).value;
  const signature = crypto
    .createHash("md5")
    .update(`${username}${apiKey}${req.body.ref_id}`)
    .digest("hex");
