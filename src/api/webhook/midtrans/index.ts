...
const main: RequestHandler = async (req, res) => {
    const body = req.body;
    console.log(`WEBHOOK ${body.payment_type} ${body.transaction_status} diterima [${body.order_id}]`);
    ...
    if (!invoice) {
        throw new BusinessError(`Invoice tidak ditemukan dengan invoice: ${body.order_id}`, ErrorType.Internal);
    }

    res.send({ status: true });

    if (body.transaction_status === "settlement" && invoice.status !== InvoiceStatuses.PAID) {
        console.log("meow");
