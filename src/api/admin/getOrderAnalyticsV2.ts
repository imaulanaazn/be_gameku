...
const main: RequestHandler = async (req, res) => {
    ...
    for (const data of ordersCount) {
        ...
        const prevOrder = prevOrdersCount.find((order) => order.status === data.status);
        if (!statusCount[category]) {
            const percentageChange = Math.round(((data.total - prevOrder.total) / data.total) * 100);
        ...
        statusCount[category].totalBefore = statusCount[category].totalBefore + prevOrder.total;
        const percentageChange = Math.round(
            ((statusCount[category].total - statusCount[category].totalBefore) / statusCount[category].total) * 100,
        );
