...
import { fn, literal, col, Op, FindAttributeOptions, Order, GroupOption } from "sequelize";
...
export class OrderService extends MainService<OrderEntity, OrderDto> {
    ...
    async countOrderGroupingByStatus(data: IPeriode): Promise<{ status: string; total: number }[]> {
        ...
        const countOrders: { dataValues: { status: string; total: number } }[] = (await this.model.findAll({
        ...
        for (const statuses of mappingStatus) {
            const check = countOrders.find((item) => item.dataValues.status === statuses.status);
            if (check) {
